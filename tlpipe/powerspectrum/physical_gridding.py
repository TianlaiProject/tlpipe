...
def physical_grid_largeangle(input_array_list, refinement=2, pad=5, order=2, 
        info_only=False, cube_force=None, feedback=False):
    ...
    if not isinstance(input_array_list, list):
        input_array_list = [input_array_list, ]
    ...
    for input_array in input_array_list:
        ...
        freq_axis.append(input_array.get_axis('freq') / 1.e6)
        ra_axis.append(input_array.get_axis('ra'))
        dec_axis.append(input_array.get_axis('dec'))
        dec_center = input_array.info['dec_centre']
        (numz, numx, numy) = input_array.shape

    freq_axis = np.array(freq_axis).flatten()
    ra_axis = np.array(ra_axis).flatten()
    dec_axis = np.array(dec_axis).flatten()

    nu_lower, nu_upper = freq_axis.min(), freq_axis.max()
    ra_fact = sp.cos(sp.pi * dec_center / 180.0)
    thetax, thetay = np.ptp(ra_axis), np.ptp(dec_axis)
    map_ra_centre = ra_axis.min() + 0.5 * thetax
    # For large angle
    ...
    if cube_force != None:
        n = (phys_dim/float(cube_force)).astype(int)
        phys_dim = n * cube_force
        c_delta = c2 - c1 - phys_dim[0]
        c2 -= 0.5 * c_delta
        c1 += 0.5 * c_delta
    elif len(input_array_list) > 1:
        print "Error, for multi-maps, cub_force needed"
        exit()
    ...
    for i in range(n[0]):
        ...
        for input_array in input_array_list:
            ...
            dra = input_array.info['ra_delta']
            dthetax = 180.*2.*np.arcsin(ra_fact*np.sin(0.5*np.pi*dra/180.))/np.pi 

            ra_centre = (map_ra_centre - ra_axis[0])/dra
            #ra_centre = 180.*2.*np.arcsin(
            #        ra_fact*np.sin(0.5*np.pi*ra_centre/180.))/np.pi 
            #ra_centre /= dthetax
            ...
            interpol_grid[1, :, :] = proper_x[i,:,:] / dthetax + ra_centre
            ...
            mask[i] += np.logical_not(np.logical_or(interpol_grid[0, :, :],
                                                interpol_grid[1, :, :],
                                                interpol_grid[2, :, :],)).astype(int)
        mask[i][mask[i]==0] = np.inf
...
def physical_grid(input_array, refinement=2, pad=5, order=2, 
        info_only=False, feedback=False, cube_force=None):
    r"""Project from freq, ra, dec into physical coordinates
