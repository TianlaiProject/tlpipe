...
def loadChains(root=None, chain_indices=None, ignore_rows=0, ignore_frac=0, 
        no_cache=False, separate_chains=False, no_stat=False, ext='.txt'):
    c = chains(root, ignore_rows=ignore_rows)
    if root is not None:
        files = c.chainFiles(root, chain_indices, ext=ext)
        c.cachefile = root + ext + '.pysamples'
        if not separate_chains and chain_indices is None and not no_cache and\
                os.path.exists(c.cachefile) and c.lastModified(files) < os.path.getmtime(c.cachefile):
            ...
            return c
        elif c.loadChains_txt(root, files):
            c.removeBurnFraction(ignore_frac)
            c.deleteFixedParams()
            if not no_stat: c.getChainsStats()
            if not separate_chains:
...
class chain():
    def __init__(self, chain_data, ignore_rows=0):
        self.setColData(chain_data)
...
class chains():
    ...
    def loadChains_txt(self, root, files):
        self.chains = []
        for fname in files:
                print fname
                self.chains.append(chain_txt(fname, self.ignore_rows))
    ...
    def loadChains(self, root, files):
        ...
        for fname in files:
            try:
                mcmc_chains = h5py.File(fname, 'r')
            except IOError:
                continue
            if 'n_steps' in mcmc_chains.keys():
                mcmc_n = mcmc_chains['n_steps'][...]
                chain_list = []
                chisq_list = []
                for i in range(mcmc_n):
                    chain_list.append(mcmc_chains['chains_%02d'%i][...])
                    chisq_list.append(mcmc_chains['chisqs_%02d'%i][...])
            else:
                chain_list = [(mcmc_chains['chains'][...]), ]
                chisq_list = [(mcmc_chains['chisqs'][...]), ]
            mcmc_chains.close()
            chain_list = np.concatenate(chain_list, axis=1)
            chisq_list = np.concatenate(chisq_list, axis=1)[:, :, None]
            weight = np.ones(chisq_list.shape)
            chain_data = np.concatenate([weight, chisq_list, chain_list], axis=-1)
            for i in range(chain_data.shape[0]):
                self.chains.append(chain(chain_data[i]))
    ...
    def removeBurnFraction(self, ignore_frac):
        for chain in self.chains:
            chain.coldata = \
                    chain.coldata[int(round(chain.coldata.shape[0] * ignore_frac)):, :]
