...
def freq_covariance(map1, map2, weight1, weight2, freq1, freq2, no_weight=False, 
    ...
    map1_flat = np.reshape(input_map1, (map1shp[0], np.product(map1shp[1:])))
    weight1_flat = np.reshape(input_weight1, (map1shp[0], np.product(map1shp[1:])))
    map2_flat = np.reshape(input_map2, (map2shp[0], np.product(map2shp[1:])))
    weight2_flat = np.reshape(input_weight2, (map2shp[0], np.product(map2shp[1:])))
